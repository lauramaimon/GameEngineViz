class WaterBody extends Sprite{
	...
	isCollidingSurface() {
		...
		for (var i = 0; i < lessonSprites.size(); i++){
			...
			if (sprite instanceof WaterBody || sprite.getVelY == undefined) {
				continue;
			}
			var spriteX = sprite.x;
			var box = sprite.getGlobalHitboxWithDimensions();
			var midSprite = spriteX + box.width/2;
			if(box.y + box.height >= window.innerHeight-this.height && box.y < window.innerHeight-this.height && sprite.getVelY() != 0){
